...
const SwapComponent = () => {
  ...
  return (
    <div className='bg-[#0c101e] w-[95%] sm:w-[35%] p-4 px-6 rounded-xl mt-40'>
      <div className='flex items-center justify-between py-4 px-1'>
        <p>Swap</p>
        <CogIcon className='h-6' />
      </div>
      <div className='relative bg-[#12192B] p-4 py-6 rounded-xl mb-2 border-[2px] border-transparent hover:border-[#1D315B]'>
        {srcTokenComp}

        <ArrowSmDownIcon
          className='absolute left-1/2 -translate-x-1/2 -bottom-6 h-10 p-1 bg-[#12192B] border-4 border-zinc-900 text-zinc-300 rounded-xl cursor-pointer hover:scale-110'
          onClick={handleReverseExchange}
        />
      </div>

      <div className='bg-[#12192B] p-4 py-6 rounded-xl mt-2 border-[2px] border-transparent hover:border-[#1D315B]'>
        {destTokenComp}
      </div>
  ...
  function getSwapBtnClassName() {
    ...
    className +=
      swapBtnText === ENTER_AMOUNT || swapBtnText === CONNECT_WALLET
        ? ' text-blue-400 bg-[#181f3a] pointer-events-none'
        : ' bg-blue-700'
